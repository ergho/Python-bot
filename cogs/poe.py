...
class Pathofexile:
    ...
    async def event_ready(self):
        ...
        await self.get_cur()
    @commands.command(name = 'tested')
    async def test(self, ctx):
        print('Wow this worked? now?!')
    ...
    async def get_cur(self, league_ids:list):
        ...
        types = ['Currency', 'Fragment', 'Oil', 'Incubator', 'Scarab', 'Fossil', 'Resonator', 'Essence', 'DivinationCard', 'Prophecy', 'BaseType', 'UniqueMap', 'Map', 'UniqueJewel', 'UniqueFlask', 'UniqueWeapon', 'UniqueArmour', 'UniqueAccessory']
        base = ['currencyoverview', 'itemoverview']
        for i in league:
            for x in base:
                for y in types:
                    #stops inherently faulty requests, this will do for now.
                    if x == base[1] and y == types[0] or x == base[1] and y == types[1]:
                        continue
                    url = f'https://poe.ninja/api/data/{x}?league={i}&type={y}'
                    async with self.bot.aiohttp_session.get(url) as resp:
                        if resp.status != 200:
